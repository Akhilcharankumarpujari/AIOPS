...
export function Tabs({
  ...
  const onChange = onValueChange !== undefined ? onValueChange : setLocalVal;
...
export function TabsList({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`inline-flex h-10 items-center justify-center rounded-md bg-secondary p-1 text-muted-foreground ${className || ''}`}>
      {children}
    </div>
  );
}
